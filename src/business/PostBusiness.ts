...
import { InsertInputPostDTO, InsertInputCommentDTO, UpdateInputDTO, LikeDislikeDTO, GetAllPostsInputDTO, GetPostsInputDTO, DeleteInputPostDTO } from "../dto/postDTO"
...
export class PostBusiness {
    ...
    public deletePost = async (input:DeleteInputPostDTO )=>{
        ...
        if(filterUserDB === undefined){
            throw new BadRequestError("Dados de publicação 'indefinidos'")
        }

        if(filterUserDB.role !== ROLE_USER.ADMIN){
            if(filterUserDB.id !== payload.id){
                throw new BadRequestError("Você não possui autorização para isso")
            }
        }
